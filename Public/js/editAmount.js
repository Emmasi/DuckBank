...
const editaAmount = async (e) => {
  ...
  const inputReduce = document.querySelector(
    `#reduce-${e.target.dataset.custid}`
  );
  const editPostItem = entries.find(
    ({ _id }) => _id === e.target.dataset.custid
  );
  const amountToChange = parseInt(inputAdd.value) - parseInt(inputReduce.value);
  const currentAmount = parseInt(editPostItem.amount);
